...
RedisKey instructorNameKey = new RedisKey("instructors:1:name");
...
// will choose the appropriate command for the Data type
// For example, when passing in an integer, it will call the 
// INCRBY command under the hood, see below:
long tempAsLong = db.StringIncrement(tempKey, 5);
...
double tempAsDouble = db.StringIncrement(tempKey, .5);
...
// a timespan that will expire the key after the allowed time.
...
string conditionalKey = "ConditionalKey";
string conditionalKeyText = "this has been set";
